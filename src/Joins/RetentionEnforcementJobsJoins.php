...
namespace BlackCat\Database\Packages\RetentionEnforcementJobs\Joins;
 ...
 */
final class RetentionEnforcementJobsJoins {
    ...
    /**
     * FK: retention_enforcement_jobs -> data_retention_policies
     * LEFT JOIN vw_data_retention_policies AS $as ON $as.id = $alias.policy_id
     * @return array{0:string,1:array<string,mixed>}
     */
    public function joinDataRetentionPolicies(string $alias = 't', string $as = 'j0'): array {
        [$alias, $as] = $this->assertAliasPair($alias, $as);
        return [' LEFT JOIN vw_data_retention_policies AS ' . $as . ' ON ' . $as . '.id = ' . $alias . '.policy_id' . ' ', []];
    }
